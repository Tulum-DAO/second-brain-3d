...
// canonical fleet vocabulary (semantic-recall-wiring-dev @ae8623618): phone/watch/web.
// Voice is iOS-ONLY (watch has no voice paths — ios-watch-dev via ob msg_0d0b0b84), so
// ios → phone is CORRECT for voice requests, not a guess. Absence always = unknown.
const canonDevice = (d) => d === 'ios' ? 'phone' : (d || 'unknown');

// device for a call: journal surface field (post :5071-restart stamping, authoritative)
// → sidecar (per-call truth) → active-surface current → origin label
// Watch presence now writes real active-surface slots (ios-watch-dev msg_9e1a4d2c), so any
// tier can report 'watch' — impossible for voice (no watch voice path): degrade to unknown.
function resolveDevice(call) {
  const r = resolveDeviceRaw(call);
  if (r.device === 'watch') return { device: 'unknown', source: r.source + ':watch-impossible' };
  return r;
}
function resolveDeviceRaw(call) {
  if (call.surface && call.surface.device) return { device: call.surface.device, source: 'journal' };
  ...
  if (sc && fs.existsSync(sc)) {
    try {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (d.device) return { device: canonDevice(d.device), source: 'sidecar' };
      }
    } catch {}
  }
  ...
  if (act && act.current && act.current.device) {
    ...
    if (age < 3600) return { device: canonDevice(act.current.device), source: 'active-surface' };
  }
