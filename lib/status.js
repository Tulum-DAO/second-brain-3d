...
// Best-effort — any capture failure or ambiguity degrades to "not draft"/"not working".
// One capture serves BOTH signals:
//   draft — the prompt line (❯ claude / > gemini / › codex) holds typed, unsubmitted text.
//   outputChanged — the pane's CONTENT (prompt/status/separator lines excluded, so typing
//     reads as draft, not work) differs from last tick's hash. This is the working-fallback
//     for runtimes that rotate transcript files (antigravity new brain/<uuid> per convo,
//     codex new rollout per convo) leaving agent-sessions' conversation_path stale.
//     Idle TUIs redraw (cursor/spinner escapes) but their captured TEXT is stable.
const PROMPT_RE = /^[❯>›]\s/;
const paneHashes = new Map();   // sess -> last content hash
function probePane(sess) {
  const out = { draft: false, outputChanged: false };
  try {
    const pane = execFileSync('tmux', ['capture-pane', '-p', '-t', sess], { encoding: 'utf8', timeout: 1500 });
    const lines = pane.split('\n');
    for (let i = lines.length - 1; i >= 0; i--) {
      if (!/^[❯>›]($|\s)/.test(lines[i])) continue;   // the input-box prompt line (bare marker = empty box)
      const typed = lines[i].slice(1).trim();
      // placeholder hints are not drafts (codex: "Ask Codex to do anything", claude: "Try ...")
      out.draft = !!typed && !/^(Ask Codex|Try )/i.test(typed);
      break;
    }
    const content = lines.filter(l => !PROMPT_RE.test(l) && !/^[─-]{20,}\s*$/.test(l)).join('\n');
    let hash = 0;
    for (let i = 0; i < content.length; i++) hash = (hash * 31 + content.charCodeAt(i)) | 0;
    const prev = paneHashes.get(sess);
    paneHashes.set(sess, hash);
    out.outputChanged = prev !== undefined && prev !== hash;
  } catch { /* dead pane / tmux hiccup → both false */ }
  return out;
}
...
function resolveAll() {
  ...
  for (const [aid, a] of Object.entries(sessions)) {
    ...
    if (!live.has(sess)) s = 'dead';
      ...
      const pane = probePane(sess);
      if (working || pane.outputChanged) s = 'working';
      else s = pane.draft ? 'draft' : 'idle';
    }
    next['agent:' + aid] = s;
  }
  return next;
