...
export function buildGraph() {
  ...
  // radial base position per cluster (well-separated pillars, alternating height for 3D depth)
  const HUB_R = 360;
  const clusterAngle = {}, clusterY = {};
  CLUSTER_ORDER.forEach((c, i) => {
    clusterAngle[c] = (i / CLUSTER_ORDER.length) * Math.PI * 2;
    clusterY[c] = ((i % 2) ? 1 : -1) * 110 + (i % 3 - 1) * 40; // stagger up/down so clusters don't overlap
  });
  // members seeded on a small sphere around their pinned hub -> pre-spread, deterministic
  const seed = (cluster, spread = 70) => {
    const a = clusterAngle[cluster];
    const u = rnd() * Math.PI * 2, v = Math.acos(2 * rnd() - 1), r = spread * (0.5 + rnd() * 0.6);
    return {
      x: Math.cos(a) * HUB_R + r * Math.sin(v) * Math.cos(u),
      y: clusterY[cluster] + r * Math.cos(v),
      z: Math.sin(a) * HUB_R + r * Math.sin(v) * Math.sin(u),
    };
  };
  ...
  for (const c of CLUSTER_ORDER) {
    ...
    add({
      ...
      val: 40, status: 'hub',
      fx: Math.cos(a) * HUB_R, fy: clusterY[c], fz: Math.sin(a) * HUB_R,
