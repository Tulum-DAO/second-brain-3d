...
export function buildGraph() {
  ...
  // ---- QUEUE hub (messages fly here as pulses) ----
  add({
    id: 'queue:bridged', cluster: 'queue', kind: 'queue', name: 'bridged',
    val: 12, status: 'live', ...seed('queue', 20),
    meta: { path: path.join(AO, 'queue/bridged'), desc: 'inter-agent message bus' },
  });
  link('hub:queue', 'queue:bridged', { kind: 'member' });
...
// map a message from/to name -> an existing node id (agent or service)
export function resolveAgentId(name, graph) {
  if (!name) return null;
  const ids = [`agent:${name}`, `service:${name}`];
  for (const id of ids) if (graph.index.has(id)) return id;
  return null;
}
