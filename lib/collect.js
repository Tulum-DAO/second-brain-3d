...
export function buildGraph() {
  ...
  const seed = (cluster, spread = 60) => {
    const a = clusterAngle[cluster];
    return {
      x: Math.cos(a) * HUB_R + (Math.random() - 0.5) * spread,
      y: clusterY[cluster] + (Math.random() - 0.5) * spread,
      z: Math.sin(a) * HUB_R + (Math.random() - 0.5) * spread,
    };
  };
